...
class FeaturedDetectionModelUtils(ModelUtils):
    ...
    def add_dict_info(self, sentences, windowSize, datasetName):
        ...
        orgBigDic = set()
        miscBigDic = set()
        with open("feature_dictionary/" + datasetName + "/personBigDic.txt", "r") as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    perBigDic.add(line)
        with open("feature_dictionary/" + datasetName + "/locationBigDic.txt", "r") as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    locBigDic.add(line)
        with open("feature_dictionary/" + datasetName + "/organizationBigDic.txt", "r") as fw:
            for line in fw:
                line = line.strip()
                if len(line) > 0:
                    orgBigDic.add(line)
        if self.dp.dataset != "muc" and self.dp.dataset != "twitter":
            with open("feature_dictionary/" + datasetName + "/miscBigDic.txt", "r") as fw:
                for line in fw:
                    line = line.strip()
                    if len(line) > 0:
                        miscBigDic.add(line)
